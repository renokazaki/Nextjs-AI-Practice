...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await generateText({
      ...
      tools,
      system:
        "AIアシスタントです。ユーザーの質問に答えるために、必ずウェブ検索ツールを使用してください。\n\n重要な指示:\n1. 検索クエリには「日本」「日本語」などのキーワードを含めてください\n2. include_domainsパラメータは使用しないでください（エラーの原因となります）\n3. 検索結果を元に、わかりやすく日本語で回答してください\n4. 検索ツールを使わずに推測で答えることは避けてください\n5. 検索結果が見つからない場合は、その旨を明確に伝えてください\n\n例: 「AI技術 トレンド 日本」「最新ニュース 日本」のように検索してください",
      prompt: query,
    });
